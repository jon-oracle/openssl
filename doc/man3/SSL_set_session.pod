...
again during L<SSL_connect()|SSL_connect(3)>. Whether the session was reused can be queried
with the L<SSL_session_reused(3)> call.

If there is already a session set inside B<ssl> (because it was set with
SSL_set_session() before or because the same B<ssl> was already used for
a connection), L<SSL_SESSION_free()|SSL_SESSION_free(3)> will be called for that session. If that old
