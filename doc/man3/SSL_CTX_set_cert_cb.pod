...
An application will typically call L<SSL_use_certificate()|SSL_CTX_use_certificate(3)> and
L<SSL_use_PrivateKey()|SSL_CTX_use_certificate(3)> to set the end entity certificate and private key.
It can add intermediate and optionally the root CA certificates using
L<SSL_add1_chain_cert()|SSL_CTX_add1_chain_cert(3)>.
