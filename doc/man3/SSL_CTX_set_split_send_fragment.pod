...
amount of data provided to the SSL_write_ex() or L<SSL_write()|SSL_write(3)> call divided by
B<split_send_fragment>.

For example if B<split_send_fragment> is set to 2000 and B<max_pipelines> is 4
then:

L<SSL_write()|SSL_write(3)>/SSL_write_ex called with 0-2000 bytes == 1 pipeline used

L<SSL_write()|SSL_write(3)>/SSL_write_ex called with 2001-4000 bytes == 2 pipelines used

L<SSL_write()|SSL_write(3)>/SSL_write_ex called with 4001-6000 bytes == 3 pipelines used

L<SSL_write()|SSL_write(3)>/SSL_write_ex called with 6001+ bytes == 4 pipelines used
...
connection. Setting B<read_ahead> can impact the behaviour of the L<SSL_pending()|SSL_pending(3)>
