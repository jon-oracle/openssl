...
L<SSL_CTX_set_mode(3)> and L<SSL_set_mode()|SSL_CTX_set_mode(3)> functions.

During a handshake, the option settings of the SSL object are used. When
a new SSL object is created from a context using L<SSL_new()|SSL_new(3)>, the current
option setting is copied. Changes to B<ctx> do not affect already created
SSL objects. L<SSL_clear()|SSL_clear(3)> does not affect the settings.
