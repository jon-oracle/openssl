...
L<SSL_CTX_set_client_cert_cb()|SSL_CTX_set_client_cert_cb(3)> if no certificate is provided at initialization.
...
successfully authenticate. This must be called before L<SSL_connect()|SSL_connect(3)>.
...
sent to the client, this can be done with SSL_do_handshake() or L<SSL_write_ex()|SSL_write(3)>.
