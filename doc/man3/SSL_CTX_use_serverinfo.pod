...
L<SSL_CTX_use_certificate()|SSL_CTX_use_certificate(3)>, the serverinfo extension will be loaded into the
...
each time a certificate is loaded via a call to L<SSL_CTX_use_certificate()|SSL_CTX_use_certificate(3)>.
