...
B<read_ahead> can impact the behaviour of the L<SSL_pending()|SSL_pending(3)> function
(see L<SSL_pending(3)>).

Since L<SSL_read()|SSL_read(3)> can return B<SSL_ERROR_WANT_READ> for non-application data
records, and L<SSL_has_pending()|SSL_pending(3)> can't tell the difference between processed and
unprocessed data, it's recommended that if read ahead is turned on that
B<SSL_MODE_AUTO_RETRY> is not turned off using L<SSL_CTX_clear_mode()|SSL_CTX_set_mode(3)>.
...
connection) after the SSL connection is finished using L<SSL_shutdown()|SSL_shutdown(3)> reading
