...
occurred on a connection i.e. if L<SSL_get_error()|SSL_get_error(3)> has returned SSL_ERROR_SYSCALL
or SSL_ERROR_SSL.
...
It is not possible to call L<SSL_write()|SSL_write(3)> after calling SSL_shutdown().
...
to continue the handshake. In this case a call to L<SSL_get_error()|SSL_get_error(3)> with the
...
However, it is recommended to wait for it using L<SSL_read()|SSL_read(3)> instead.
...
L<SSL_read()|SSL_read(3)> should be called until all data is received.
L<SSL_read()|SSL_read(3)> will indicate the end of the peer data by returning <= 0
and L<SSL_get_error()|SSL_get_error(3)> returning SSL_ERROR_ZERO_RETURN.
...
L<SSL_read()|SSL_read(3)> will return <= 0 in that case, and SSL_get_error() will return
...
L<SSL_get_shutdown()|SSL_set_shutdown(3)> (see also L<SSL_set_shutdown(3)> call.
