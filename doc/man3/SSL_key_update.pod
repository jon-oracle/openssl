...
until the next time an IO operation such as SSL_read_ex() or L<SSL_write_ex()|SSL_write(3)>
takes place on the connection. Alternatively L<SSL_do_handshake()|SSL_do_handshake(3)> can be called to
...
such as SSL_read_ex() or L<SSL_write_ex()|SSL_write(3)> takes place on the connection a check
