...
The ciphersuite can be set via a call to <L<SSL_SESSION_set_cipher()|SSL_SESSION_get0_cipher(3)>3)>. The
handshake digest of an SSL_CIPHER object can be checked using
<L<SSL_CIPHER_get_handshake_digest()|SSL_CIPHER_get_name(3)>3)>.
