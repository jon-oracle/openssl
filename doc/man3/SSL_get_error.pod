...
statement) for a preceding call to SSL_connect(), SSL_accept(), L<SSL_do_handshake()|SSL_do_handshake(3)>,
SSL_read_ex(), SSL_read(), SSL_peek_ex(), SSL_peek(), L<SSL_shutdown()|SSL_shutdown(3)>,
SSL_write_ex() or L<SSL_write()|SSL_write(3)> on B<ssl>.  The value returned by that TLS/SSL I/O
