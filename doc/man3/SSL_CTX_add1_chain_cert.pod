...
function such as L<SSL_CTX_use_certificate()|SSL_CTX_use_certificate(3)>.
...
structures when L<SSL_new()|SSL_new(3)> is called. SSL structures will not be affected
...
L<SSL_CTX_use_certificate_chain_file()|SSL_CTX_use_certificate(3)> then call SSL_CTX_build_cert_chain()
...
using L<SSL_CTX_add_extra_chain_cert()|SSL_CTX_add_extra_chain_cert(3)> will be used.
